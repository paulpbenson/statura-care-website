...
import { blogPosts, blogTags, toSlug, getTagBySlug } from "@/data/blog";
...
export async function generateMetadata({
  ...
  if (!tagName) return { title: "Tag Not Found — Statura Care" };

  const title = `Articles tagged '${tagName}' — Statura Care Blog`;
  const description = `Browse all articles tagged '${tagName}' from Statura Care. Expert guidance for Australian aged care providers.`;

  return {
    title,
    description,
    alternates: { canonical: `https://statura.care/blog/tag/${slug}` },
    robots: { index: true, follow: true },
    openGraph: {
      title,
      description,
      url: `https://statura.care/blog/tag/${slug}`,
      type: "website",
      locale: "en_AU",
    },
  };
...
export default async function BlogTagPage({
  ...
  const filteredPosts = tagName
    ? [...blogPosts]
        .filter((p) => p.tags.some((t) => toSlug(t) === slug))
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: tagName || slug },
    ],
  };
          ...
          <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
            <div className="max-w-3xl">
              ...
              <nav className="flex items-center gap-1.5 text-sm text-slate-400 mb-6">
                ...
                <ChevronRight className="w-3.5 h-3.5" />
                <span className="text-slate-500">{tagName || slug}</span>
              </nav>
              ...
              <p className="mt-6 text-lg lg:text-xl text-slate-300 leading-relaxed max-w-2xl">
                {filteredPosts.length} {filteredPosts.length === 1 ? "article" : "articles"} tagged with this topic.
              </p>
            </div>
          </div>
        </section>
